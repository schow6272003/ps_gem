  ...
  describe "#find"  do
    ...
    context "when valid request params" do 
      ...
      it "expects to return 1 record" do
        expect(subject[:body].count).to eq 1
      end
      
      it "expects cbsa code to be 11260" do
       expect(subject[:body][0]["cbsa_id"]).to eq "11260"
      end 

      it "expects to include zipcodes" do
        expect(subject[:body][0]["zip_code"]).to include(99501,99502,99503)
      end 
      
      it "expects to include msa name" do
        expect(subject[:body][0]["name"]).to eq "Anchorage, AK"
      end

      it "expects to include 2014 pop estimate" do 
        expect(subject[:body][0]["pop_estimate"][4]["number"]).to eq "398642"
      end
      it "expects to include 2015 pop estimate" do 
        expect(subject[:body][0]["pop_estimate"][5]["number"]).to eq "399790"
      end
    end  
    ...
    context "when search using zipcodes" do
      ...
      it "expects first record to have zipcode 48813" do
        expect(subject[:body][0]["zip_code"]).to include(48813)
      end

      it "expects first record to have zipcode 5482" do
        expect(subject[:body][1]["zip_code"]).to include(5482)
      end
    end  
    ...
    context "when search msa name by keyword" do
      ...
      it "expects to include 'Burlington' on msa name" do
        expect(subject[:body][0]["name"]).to include("Burlington")
      end
    end 
